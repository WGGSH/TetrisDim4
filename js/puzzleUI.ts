  ...
  public update(): void{
    if (Input.MouseDown) {
      ...
      for (let i: number = 0; i < 4; i++){
        ...
        fill(255);
        targetPos.set(
          width / 2 + Puzzle.UI_HEIGHT / 3 * Math.cos(radians(i * 90 - 60 * this.puzzle.Position.w - 60) - Camera.AngleX),
          height / 8 * 7 + Puzzle.UI_HEIGHT / 3 * Math.sin(radians(i * 90 - 60 * this.puzzle.Position.w - 60) - Camera.AngleX)
        );
